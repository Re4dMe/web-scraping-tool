export class TranslationDBObject {
    ...
    promisifiedQuery(query_string: string) {
        return new Promise((resolve, reject) => {
            this.pool.query(query_string, (error: any, results: any) => {
                if (error) {
                    console.log(`Error while doing pool query: ${error.toString()}`);
                    reject(error);
                }
                resolve(results)
            });
    ...
    async createData(table: string, var_string: string, chinese_string: string, english_string: string) {
        //TODO get format of data by table
        let query_string = `INSERT INTO ${table} (var_string, chinese_string, english_string) VALUES (${var_string}, ${chinese_string}, ${english_string}) RETURNING *`;
